...
class Packet {
    static type: {
        AUTH: number; AUTH_RES: number; COMMAND: number; COMMAND_RES: number;
     ...
     */
    static write (id: number, type: number, payload: string | DataView | ArrayBuffer) {
        // Length of payload in bytes when encoded in ASCII
        const length = Buffer.byteLength(payload.toString(), 'ascii');
        ...
        buffer.write(payload.toString(), 12, 'ascii'); // Payload
     ...
     */
    static read (packet: { readInt32LE: (arg0: number) => any; length: number; readInt16LE: (arg0: number) => any; toString: (arg0: string, arg1: number, arg2: number) => any; }) {
